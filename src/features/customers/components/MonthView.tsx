...
const MonthView = ({ currentDate, events, today }: MonthViewProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="grid grid-cols-7 border-b border-border flex-shrink-0">
        {WEEK_DAYS.map((d) => (
          <div key={d} className="text-center py-2 text-xs text-muted-foreground font-medium border-l border-border first:border-l-0">
            {d}
          </div>
        ))}
      </div>
            ...
            >
              ...
              <div className="space-y-0.5">
                {dayEvents.slice(0, 3).map((event) => (
                  <div
                    key={event.id}
                    className={`text-[10px] px-1 py-0.5 rounded truncate text-foreground cursor-pointer ${eventColorClasses[event.color]}`}
                  >
                    {event.title}
                  </div>
                ))}
