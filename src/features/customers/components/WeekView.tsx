...
const WeekView = ({ currentDate, events, today }: WeekViewProps) => {
  const weekStart = startOfWeek(currentDate, { weekStartsOn: 0 });
  const weekDays = Array.from({ length: 7 }, (_, i) => addDays(weekStart, i));

  const getEventsForDayAndHour = (day: Date, hour: number) =>
    events.filter((e) => isSameDay(e.date, day) && e.startHour === hour);
                ...
                >
                  {dayEvents.map((event) => {
                    ...
                    return (
                      <div
                        key={event.id}
                        className={`absolute inset-x-0.5 top-0 rounded px-1.5 py-0.5 text-xs text-foreground z-10 overflow-hidden cursor-pointer ${eventColorClasses[event.color]}`}
